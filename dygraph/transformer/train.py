...
def do_train(args):
    if args.use_cuda:
        trainer_count = paddle.fluid.dygraph.parallel.Env().nranks
        place = paddle.CUDAPlace(paddle.fluid.dygraph.parallel.Env(
        ).dev_id) if trainer_count > 1 else paddle.CUDAPlace(0)
    else:
        trainer_count = 1
    ...
    if trainer_count > 1:  # for multi-process gpu training
        batch_generator = paddle.fluid.contrib.reader.distributed_batch_reader(
            batch_generator)
    ...
    if random_seed is not None:
        ...
        # paddle.static.default_main_program().random_seed = random_seed
        # paddle.static.default_startup_program().random_seed = random_seed
    ...
    if args.validation_file:
        val_loader = paddle.io.DataLoader.from_generator(capacity=10)
    ...
    if args.init_from_checkpoint:
        # model_dict, opt_dict = paddle.fluid.load_dygraph(
        #     os.path.join(args.init_from_checkpoint, "transformer"))
        model_dict, opt_dict = paddle.load(
            os.path.join(args.init_from_checkpoint, "transformer"))
    ...
    if args.init_from_pretrain_model:
        model_dict, _ = paddle.fluid.load_dygraph(
    ...
    if trainer_count > 1:
        ...
        transformer = paddle.fluid.dygraph.parallel.DataParallel(transformer,
                                                                 strategy)
        ...
        for input_data in train_loader():
            ...
            if step_idx % args.save_step == 0 and step_idx != 0:
                ...
                if args.save_model and (
                        trainer_count == 1 or
                        paddle.fluid.dygraph.parallel.Env().dev_id == 0):
    ...
    if args.save_model:
        ...
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        paddle.fluid.save_dygraph(transformer.state_dict(),
                                  os.path.join(model_dir, "transformer"))
        paddle.fluid.save_dygraph(optimizer.state_dict(),
                                  os.path.join(model_dir, "transformer"))
