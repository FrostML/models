# Copyright (c) 2019 PaddlePaddle Authors. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from __future__ import print_function

import numpy as np

import paddle
import paddle.fluid as fluid
import paddle.fluid.layers as layers
from paddle.fluid.dygraph import Embedding, LayerNorm, Linear, Layer, to_variable
from paddle.fluid.dygraph.learning_rate_scheduler import LearningRateDecay


def position_encoding_init(n_position, d_pos_vec):
    """
    Generate the initial values for the sinusoid position encoding table.
    """
    channels = d_pos_vec
    position = np.arange(n_position)
    num_timescales = channels // 2
    log_timescale_increment = (np.log(float(1e4) / float(1)) /
                               (num_timescales - 1))
    inv_timescales = np.exp(np.arange(
        num_timescales)) * -log_timescale_increment
    scaled_time = np.expand_dims(position, 1) * np.expand_dims(inv_timescales,
                                                               0)
    signal = np.concatenate([np.sin(scaled_time), np.cos(scaled_time)], axis=1)
    signal = np.pad(signal, [[0, 0], [0, np.mod(channels, 2)]], 'constant')
    position_enc = signal
    return position_enc.astype("float32")


class Embedder(Layer):
    """
    Word Embedding + Position Encoding
    """

    def __init__(self, vocab_size, emb_dim, bos_idx=0):
        super(Embedder, self).__init__()

        self.word_embedder = paddle.nn.Embedding(
            num_embeddings=vocab_size,
            embedding_dim=emb_dim,
            # size=[vocab_size, emb_dim],
            padding_idx=bos_idx,
            weight_attr=fluid.ParamAttr(
                initializer=fluid.initializer.Normal(0., emb_dim**-0.5)))

    def forward(self, word):
        word_emb = self.word_embedder(word)
        return word_emb


class CrossEntropyCriterion(object):
    def __init__(self, label_smooth_eps):
        self.label_smooth_eps = label_smooth_eps

    def __call__(self, predict, label, weights):
        if self.label_smooth_eps:
            label = paddle.squeeze(label, axis=[1])
            label = paddle.nn.functional.label_smooth(
                label=paddle.nn.functional.one_hot(
                    x=label, num_classes=predict.shape[-1]),
                epsilon=self.label_smooth_eps)

        cost = paddle.nn.functional.softmax_with_cross_entropy(
            logits=predict,
            label=label,
            soft_label=True if self.label_smooth_eps else False)
        weighted_cost = cost * weights
        sum_cost = paddle.reduce_sum(weighted_cost)
        token_num = paddle.reduce_sum(weights)
        token_num.stop_gradient = True
        avg_cost = sum_cost / token_num
        return sum_cost, avg_cost, token_num


class Transformer(Layer):
    """
    model
    """

    def __init__(self,
                 src_vocab_size,
                 trg_vocab_size,
                 max_length,
                 n_layer,
                 n_head,
                 d_key,
                 d_value,
                 d_model,
                 d_inner_hid,
                 prepostprocess_dropout,
                 attention_dropout,
                 relu_dropout,
                 preprocess_cmd,
                 postprocess_cmd,
                 weight_sharing,
                 bos_id=0,
                 eos_id=1):
        super(Transformer, self).__init__()
        self.src_word_embedder = Embedder(
            vocab_size=src_vocab_size, emb_dim=d_model, bos_idx=bos_id)
        if weight_sharing:
            assert src_vocab_size == trg_vocab_size, (
                "Vocabularies in source and target should be same for weight sharing."
            )
            self.trg_word_embedder = self.src_word_embedder
        else:
            self.trg_word_embedder = Embedder(
                vocab_size=trg_vocab_size, emb_dim=d_model, bos_idx=bos_id)

        self.transformer = paddle.nn.Transformer(
            d_model=d_model,
            nhead=n_head,
            num_encoder_layers=n_layer,
            num_decoder_layers=n_layer,
            dim_feedforward=d_inner_hid,
            dropout=prepostprocess_dropout,
            activation="relu",
            attn_dropout=attention_dropout,
            act_dropout=relu_dropout,
            normalize_before=True,
            bias_attr=[False, False, True])

        self.trg_vocab_size = trg_vocab_size
        self.n_layer = n_layer
        self.n_head = n_head
        self.d_key = d_key
        self.d_value = d_value
        self.prepostprocess_dropout = prepostprocess_dropout
        self.emb_dim = d_model

        self.encoder_pos_encoder = paddle.nn.Embedding(
            num_embeddings=max_length,
            embedding_dim=self.emb_dim,
            # size=[max_length, self.emb_dim],
            weight_attr=fluid.ParamAttr(
                initializer=fluid.initializer.NumpyArrayInitializer(
                    position_encoding_init(max_length, self.emb_dim)),
                trainable=False))

        self.decoder_pos_encoder = paddle.nn.Embedding(
            num_embeddings=max_length,
            embedding_dim=self.emb_dim,
            # size=[max_length, self.emb_dim],
            weight_attr=fluid.ParamAttr(
                initializer=fluid.initializer.NumpyArrayInitializer(
                    position_encoding_init(max_length, self.emb_dim)),
                trainable=False))

        if weight_sharing:
            self.linear = lambda x: paddle.matmul(x=x,
                                                  y=self.trg_word_embedder.word_embedder.weight,
                                                  transpose_y=True)
        else:
            self.linear = Linear(
                input_dim=d_model, output_dim=trg_vocab_size, bias_attr=False)

    def forward(self, src_word, src_pos, src_slf_attn_bias, trg_word, trg_pos,
                trg_slf_attn_bias, trg_src_attn_bias):
        src = self.src_word_embedder(src_word)
        src_emb = paddle.scale(x=src, scale=self.emb_dim**0.5)
        src_pos = self.encoder_pos_encoder(src_pos)
        src_pos.stop_gradient = True
        src_emb = src_emb + src_pos
        enc_input = paddle.nn.functional.dropout(
            src_emb, p=self.prepostprocess_dropout,
            training=True) if self.prepostprocess_dropout else src_emb

        trg = self.trg_word_embedder(trg_word)
        trg_emb = paddle.scale(x=trg, scale=self.emb_dim**0.5)
        trg_pos = self.decoder_pos_encoder(trg_pos)
        trg_pos.stop_gradient = True
        trg_emb = trg_emb + trg_pos
        dec_input = paddle.nn.functional.dropout(
            trg_emb, p=self.prepostprocess_dropout,
            training=True) if self.prepostprocess_dropout else trg_emb

        # src, tgt, src_mask=None, tgt_mask=None, memory_mask=None
        dec_output = self.transformer(
            enc_input,
            dec_input,
            src_mask=src_slf_attn_bias,
            tgt_mask=trg_slf_attn_bias,
            memory_mask=trg_src_attn_bias)
        dec_output = paddle.reshape(
            dec_output,
            shape=[-1, dec_output.shape[-1]], )
        predict = self.linear(dec_output)
        return predict

    def beam_search_v2(self,
                       src_word,
                       src_pos,
                       src_slf_attn_bias,
                       trg_word,
                       trg_src_attn_bias,
                       bos_id=0,
                       eos_id=1,
                       beam_size=4,
                       max_len=None,
                       alpha=0.6):
        """
        Beam search with the alive and finished two queues, both have a beam size
        capicity separately. It includes `grow_topk` `grow_alive` `grow_finish` as
        steps.
        1. `grow_topk` selects the top `2*beam_size` candidates to avoid all getting
        EOS.
        2. `grow_alive` selects the top `beam_size` non-EOS candidates as the inputs
        of next decoding step.
        3. `grow_finish` compares the already finished candidates in the finished queue
        and newly added finished candidates from `grow_topk`, and selects the top
        `beam_size` finished candidates.
        """

        def expand_to_beam_size(tensor, beam_size):
            tensor = paddle.reshape(
                tensor, shape=[tensor.shape[0], 1] + tensor.shape[1:])
            tile_dims = tensor.shape
            tile_dims[1] = beam_size
            return paddle.expand(tensor, tile_dims)

        def merge_beam_dim(tensor):
            return paddle.reshape(tensor, shape=[-1] + tensor.shape[2:])

        # run encoder
        src = self.src_word_embedder(src_word)
        src_emb = paddle.scale(x=src, scale=self.emb_dim**0.5)
        src_pos = self.encoder_pos_encoder(src_pos)
        src_pos.stop_gradient = True
        src_emb = src_emb + src_pos
        enc_input = paddle.nn.functional.dropout(
            src_emb, p=self.prepostprocess_dropout,
            training=False) if self.prepostprocess_dropout else src_emb
        enc_output = self.transformer.encoder(enc_input, src_slf_attn_bias)
        # enc_output = self.encoder(src_word, src_pos, src_slf_attn_bias)

        # constant number
        inf = float(1. * 1e7)
        batch_size = enc_output.shape[0]
        max_len = (enc_output.shape[1] + 20) if max_len is None else max_len

        ### initialize states of beam search ###
        ## init for the alive ##
        initial_log_probs = to_variable(
            np.array(
                [[0.] + [-inf] * (beam_size - 1)], dtype="float32"))
        alive_log_probs = paddle.expand(initial_log_probs,
                                        [batch_size, beam_size])
        alive_seq = to_variable(
            np.tile(
                np.array(
                    [[[bos_id]]], dtype="int64"), (batch_size, beam_size, 1)))

        ## init for the finished ##
        finished_scores = to_variable(
            np.array(
                [[-inf] * beam_size], dtype="float32"))
        finished_scores = paddle.expand(finished_scores,
                                        [batch_size, beam_size])
        finished_seq = to_variable(
            np.tile(
                np.array(
                    [[[bos_id]]], dtype="int64"), (batch_size, beam_size, 1)))
        finished_flags = paddle.zeros_like(finished_scores)

        ### initialize inputs and states of transformer decoder ###
        ## init inputs for decoder, shaped `[batch_size*beam_size, ...]`
        trg_word = paddle.reshape(
            alive_seq[:, :, -1], shape=[batch_size * beam_size, 1])
        trg_src_attn_bias = merge_beam_dim(
            expand_to_beam_size(trg_src_attn_bias, beam_size))
        enc_output = merge_beam_dim(expand_to_beam_size(enc_output, beam_size))
        ## init states (caches) for transformer, need to be updated according to selected beam
        caches = self.transformer.decoder.gen_cache(enc_output)

        def update_states(caches, beam_idx, beam_size):
            for cache in caches:  # no need to update static_kv
                for c in cache:
                    if isinstance(c, paddle.nn.MultiHeadAttention.Cache):
                        paddle.fluid.layers.assign(
                            gather_2d_by_gather(c.k, beam_idx, beam_size,
                                                batch_size, False), c.k)
                        paddle.fluid.layers.assign(
                            gather_2d_by_gather(c.v, beam_idx, beam_size,
                                                batch_size, False), c.v)
            return caches

        def gather_2d_by_gather(tensor_nd,
                                beam_idx,
                                beam_size,
                                batch_size,
                                need_flat=True):
            batch_idx = paddle.arange(
                0, batch_size, 1, dtype="int64") * beam_size
            flat_tensor = merge_beam_dim(tensor_nd) if need_flat else tensor_nd
            batch_idx = paddle.tensor.unsqueeze(batch_idx, [1])
            idx = paddle.reshape(paddle.add(beam_idx, batch_idx), shape=[-1])
            new_flat_tensor = paddle.gather(flat_tensor, idx)
            new_tensor_nd = paddle.reshape(
                new_flat_tensor,
                shape=[batch_size, beam_idx.shape[1]] +
                tensor_nd.shape[2:]) if need_flat else new_flat_tensor
            return new_tensor_nd

        def early_finish(alive_log_probs, finished_scores,
                         finished_in_finished):
            max_length_penalty = np.power(((5. + max_len) / 6.), alpha)
            # The best possible score of the most likely alive sequence
            lower_bound_alive_scores = alive_log_probs[:,
                                                       0] / max_length_penalty

            # Now to compute the lowest score of a finished sequence in finished
            # If the sequence isn't finished, we multiply it's score by 0. since
            # scores are all -ve, taking the min will give us the score of the lowest
            # finished item.
            lowest_score_of_fininshed_in_finished = paddle.reduce_min(
                finished_scores * finished_in_finished, 1)
            # If none of the sequences have finished, then the min will be 0 and
            # we have to replace it by -ve INF if it is. The score of any seq in alive
            # will be much higher than -ve INF and the termination condition will not
            # be met.
            lowest_score_of_fininshed_in_finished += (
                1. - paddle.reduce_max(finished_in_finished, 1)) * -inf
            bound_is_met = paddle.reduce_all(
                paddle.greater_than(lowest_score_of_fininshed_in_finished,
                                    lower_bound_alive_scores))

            return bound_is_met

        def grow_topk(i, logits, alive_seq, alive_log_probs, states):
            logits = paddle.reshape(logits, [batch_size, beam_size, -1])
            candidate_log_probs = paddle.log(
                paddle.nn.functional.softmax(
                    logits, axis=2))
            alive_log_probs = paddle.tensor.unsqueeze(alive_log_probs, [2])
            log_probs = paddle.add(candidate_log_probs, alive_log_probs)

            length_penalty = np.power(5.0 + (i + 1.0) / 6.0, alpha)
            curr_scores = log_probs / length_penalty
            flat_curr_scores = paddle.reshape(curr_scores, [batch_size, -1])

            topk_scores, topk_ids = paddle.topk(
                flat_curr_scores, k=beam_size * 2)

            topk_log_probs = topk_scores * length_penalty

            topk_beam_index = topk_ids // self.trg_vocab_size
            topk_ids = topk_ids % self.trg_vocab_size

            # use gather as gather_nd, TODO: use gather_nd
            topk_seq = gather_2d_by_gather(alive_seq, topk_beam_index,
                                           beam_size, batch_size)
            topk_seq = paddle.concat(
                [topk_seq, paddle.reshape(topk_ids, topk_ids.shape + [1])],
                axis=2)
            states = update_states(states, topk_beam_index, beam_size)
            eos = paddle.fill_constant(
                shape=topk_ids.shape, dtype="int64", value=eos_id)
            topk_finished = paddle.cast(paddle.equal(topk_ids, eos), "float32")

            #topk_seq: [batch_size, 2*beam_size, i+1]
            #topk_log_probs, topk_scores, topk_finished: [batch_size, 2*beam_size]
            return topk_seq, topk_log_probs, topk_scores, topk_finished, states

        def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished,
                       states):
            curr_scores += curr_finished * -inf
            _, topk_indexes = paddle.topk(curr_scores, k=beam_size)
            alive_seq = gather_2d_by_gather(curr_seq, topk_indexes,
                                            beam_size * 2, batch_size)
            alive_log_probs = gather_2d_by_gather(curr_log_probs, topk_indexes,
                                                  beam_size * 2, batch_size)
            states = update_states(states, topk_indexes, beam_size * 2)

            return alive_seq, alive_log_probs, states

        def grow_finished(finished_seq, finished_scores, finished_flags,
                          curr_seq, curr_scores, curr_finished):
            # finished scores
            finished_seq = paddle.concat(
                [
                    finished_seq, paddle.fill_constant(
                        shape=[batch_size, beam_size, 1],
                        dtype="int64",
                        value=eos_id)
                ],
                axis=2)
            # Set the scores of the unfinished seq in curr_seq to large negative
            # values
            curr_scores += (1. - curr_finished) * -inf
            # concatenating the sequences and scores along beam axis
            curr_finished_seq = paddle.concat([finished_seq, curr_seq], axis=1)
            curr_finished_scores = paddle.concat(
                [finished_scores, curr_scores], axis=1)
            curr_finished_flags = paddle.concat(
                [finished_flags, curr_finished], axis=1)
            _, topk_indexes = paddle.topk(curr_finished_scores, k=beam_size)
            finished_seq = gather_2d_by_gather(curr_finished_seq, topk_indexes,
                                               beam_size * 3, batch_size)
            finished_scores = gather_2d_by_gather(
                curr_finished_scores, topk_indexes, beam_size * 3, batch_size)
            finished_flags = gather_2d_by_gather(
                curr_finished_flags, topk_indexes, beam_size * 3, batch_size)
            return finished_seq, finished_scores, finished_flags

        for i in range(max_len):
            trg_pos = paddle.fill_constant(
                shape=trg_word.shape, dtype="int64", value=i)
            trg = self.trg_word_embedder(trg_word)
            trg_emb = paddle.scale(x=trg, scale=self.emb_dim**0.5)
            trg_pos = self.decoder_pos_encoder(trg_pos)
            trg_pos.stop_gradient = True
            trg_emb = trg_emb + trg_pos
            dec_input = paddle.nn.functional.dropout(
                trg_emb, p=self.prepostprocess_dropout,
                training=False) if self.prepostprocess_dropout else trg_emb

            logits, caches = self.transformer.decoder(
                dec_input, enc_output, None, trg_src_attn_bias, caches)
            logits = paddle.reshape(
                logits,
                shape=[-1, logits.shape[-1]], )
            logits = self.linear(logits)

            # logits = self.decoder(trg_word, trg_pos, None, trg_src_attn_bias,
            #                       enc_output, caches)
            topk_seq, topk_log_probs, topk_scores, topk_finished, states = grow_topk(
                i, logits, alive_seq, alive_log_probs, caches)
            alive_seq, alive_log_probs, states = grow_alive(
                topk_seq, topk_scores, topk_log_probs, topk_finished, states)
            finished_seq, finished_scores, finished_flags = grow_finished(
                finished_seq, finished_scores, finished_flags, topk_seq,
                topk_scores, topk_finished)
            trg_word = paddle.reshape(
                alive_seq[:, :, -1], shape=[batch_size * beam_size, 1])

            if early_finish(alive_log_probs, finished_scores,
                            finished_flags).numpy():
                break

        return finished_seq, finished_scores

    def beam_search(self,
                    src_word,
                    src_pos,
                    src_slf_attn_bias,
                    trg_word,
                    trg_src_attn_bias,
                    bos_id=0,
                    eos_id=1,
                    beam_size=4,
                    max_len=256):
        if beam_size == 1:
            return self._greedy_search(
                src_word,
                src_pos,
                src_slf_attn_bias,
                trg_word,
                trg_src_attn_bias,
                bos_id=bos_id,
                eos_id=eos_id,
                max_len=max_len)
        else:
            return self._beam_search(
                src_word,
                src_pos,
                src_slf_attn_bias,
                trg_word,
                trg_src_attn_bias,
                bos_id=bos_id,
                eos_id=eos_id,
                beam_size=beam_size,
                max_len=max_len)

    def _beam_search(self,
                     src_word,
                     src_pos,
                     src_slf_attn_bias,
                     trg_word,
                     trg_src_attn_bias,
                     bos_id=0,
                     eos_id=1,
                     beam_size=4,
                     max_len=256):
        def expand_to_beam_size(tensor, beam_size):
            tensor = paddle.reshape(
                tensor, shape=[tensor.shape[0], 1] + tensor.shape[1:])
            tile_dims = tensor.shape
            tile_dims[1] = beam_size
            return paddle.expand(tensor, tile_dims)

        def merge_batch_beams(tensor):
            return paddle.reshape(
                tensor,
                shape=[tensor.shape[0] * tensor.shape[1]] + tensor.shape[2:])

        def split_batch_beams(tensor):
            return paddle.reshape(
                tensor, shape=[-1, beam_size] + list(tensor.shape[1:]))

        def mask_probs(probs, finished, noend_mask_tensor):
            # TODO: use where_op
            finished = paddle.cast(finished, dtype=probs.dtype)
            finished = paddle.tensor.unsqueeze(finished, [2])
            finished_shape = finished.shape
            probs = paddle.multiply(
                paddle.expand(finished, [
                    finished_shape[0], finished_shape[1], self.trg_vocab_size
                ]), noend_mask_tensor) - paddle.multiply(probs, (finished - 1))
            return probs

        def gather(x, indices, batch_pos):
            topk_coordinates = paddle.tensor.stack([batch_pos, indices], axis=2)
            return paddle.gather_nd(x, topk_coordinates)

        def update_states(func, caches):
            for cache in caches:  # no need to update static_kv
                for c in cache:
                    if isinstance(c, paddle.nn.MultiHeadAttention.Cache):
                        paddle.fluid.layers.assign(func(c.k), c.k)
                        paddle.fluid.layers.assign(func(c.v), c.v)
            return caches

        # run encoder
        src = self.src_word_embedder(src_word)
        src_emb = paddle.scale(x=src, scale=self.emb_dim**0.5)
        src_pos = self.encoder_pos_encoder(src_pos)
        # src_pos.stop_gradient = True
        src_emb = src_emb + src_pos
        enc_input = paddle.nn.functional.dropout(
            src_emb, p=self.prepostprocess_dropout,
            training=False) if self.prepostprocess_dropout else src_emb
        enc_output = self.transformer.encoder(enc_input, src_slf_attn_bias)

        # constant number
        inf = float(1. * 1e7)
        batch_size = enc_output.shape[0]
        max_len = (enc_output.shape[1] + 20) if max_len is None else max_len
        vocab_size_tensor = paddle.fill_constant(
            shape=[1], dtype="int64", value=self.trg_vocab_size)
        end_token_tensor = to_variable(
            np.full(
                [batch_size, beam_size], eos_id, dtype="int64"))
        noend_array = [-inf] * self.trg_vocab_size
        noend_array[eos_id] = 0
        noend_mask_tensor = to_variable(np.array(noend_array, dtype="float32"))
        batch_pos = paddle.expand(
            paddle.tensor.unsqueeze(
                to_variable(np.arange(
                    0, batch_size, 1, dtype="int64")), [1]),
            [batch_size, beam_size])

        predict_ids = []
        parent_ids = []
        ### initialize states of beam search ###
        log_probs = to_variable(
            np.array(
                [[0.] + [-inf] * (beam_size - 1)] * batch_size,
                dtype="float32"))
        finished = to_variable(
            np.full(
                [batch_size, beam_size], 0, dtype="bool"))
        ### initialize inputs and states of transformer decoder ###
        ## init inputs for decoder, shaped `[batch_size*beam_size, ...]`
        trg_word = paddle.fill_constant(
            shape=[batch_size * beam_size, 1], dtype="int64", value=bos_id)
        trg_pos = paddle.zeros_like(trg_word)
        trg_src_attn_bias = merge_batch_beams(
            expand_to_beam_size(trg_src_attn_bias, beam_size))
        enc_output = merge_batch_beams(
            expand_to_beam_size(enc_output, beam_size))
        ## init states (caches) for transformer, need to be updated according to selected beam
        caches = self.transformer.decoder.gen_cache(enc_output)

        for i in range(max_len):
            trg_pos = paddle.fill_constant(
                shape=trg_word.shape, dtype="int64", value=i)
            caches = update_states(  # can not be reshaped since the 0 size
                lambda x: x if i == 0 else merge_batch_beams(x), caches)

            trg = self.trg_word_embedder(trg_word)
            trg_emb = paddle.scale(x=trg, scale=self.emb_dim**0.5)
            trg_pos = self.decoder_pos_encoder(trg_pos)
            # trg_pos.stop_gradient = True
            trg_emb = trg_emb + trg_pos
            dec_input = paddle.nn.functional.dropout(
                trg_emb, p=self.prepostprocess_dropout,
                training=False) if self.prepostprocess_dropout else trg_emb

            logits, caches = self.transformer.decoder(
                dec_input, enc_output, None, trg_src_attn_bias, caches)
            logits = paddle.reshape(
                logits,
                shape=[-1, logits.shape[-1]], )
            logits = self.linear(logits)

            caches = update_states(split_batch_beams, caches)

            step_log_probs = split_batch_beams(
                paddle.log(paddle.nn.functional.softmax(logits)))
            step_log_probs = mask_probs(step_log_probs, finished,
                                        noend_mask_tensor)

            log_probs = paddle.tensor.unsqueeze(log_probs, axis=2)

            log_probs = paddle.add(x=step_log_probs, y=log_probs)
            log_probs = paddle.reshape(
                log_probs, shape=[-1, beam_size * self.trg_vocab_size])
            scores = log_probs
            topk_scores, topk_indices = paddle.topk(x=scores, k=beam_size)
            beam_indices = paddle.floor_divide(topk_indices, vocab_size_tensor)
            token_indices = paddle.remainder(topk_indices, vocab_size_tensor)

            # update states
            caches = update_states(lambda x: gather(x, beam_indices, batch_pos),
                                   caches)
            log_probs = gather(log_probs, topk_indices, batch_pos)
            finished = gather(finished, beam_indices, batch_pos)
            finished = paddle.logical_or(
                finished, paddle.equal(token_indices, end_token_tensor))
            trg_word = paddle.reshape(token_indices, shape=[-1, 1])

            predict_ids.append(token_indices)
            parent_ids.append(beam_indices)

            if paddle.reduce_all(finished).numpy():
                break

        predict_ids = paddle.tensor.stack(predict_ids, axis=0)
        parent_ids = paddle.tensor.stack(parent_ids, axis=0)
        finished_seq = paddle.transpose(
            paddle.nn.gather_tree(predict_ids, parent_ids), [1, 2, 0])
        finished_scores = topk_scores

        return finished_seq, finished_scores

    def _greedy_search(self,
                       src_word,
                       src_pos,
                       src_slf_attn_bias,
                       trg_word,
                       trg_src_attn_bias,
                       bos_id=0,
                       eos_id=1,
                       max_len=256):
        # run encoder
        src = self.src_word_embedder(src_word)
        src_emb = paddle.scale(x=src, scale=self.emb_dim**0.5)
        src_pos = self.encoder_pos_encoder(src_pos)
        src_pos.stop_gradient = True
        src_emb = src_emb + src_pos
        enc_input = paddle.nn.functional.dropout(
            src_emb, p=self.prepostprocess_dropout,
            training=False) if self.prepostprocess_dropout else src_emb
        enc_output = self.transformer.encoder(enc_input, src_slf_attn_bias)

        # constant number
        batch_size = enc_output.shape[0]
        max_len = (enc_output.shape[1] + 20) if max_len is None else max_len
        end_token_tensor = paddle.fill_constant(
            shape=[batch_size, 1], dtype="int64", value=eos_id)

        predict_ids = []
        log_probs = paddle.fill_constant(
            shape=[batch_size, 1], dtype="float32", value=0)
        trg_word = paddle.fill_constant(
            shape=[batch_size, 1], dtype="int64", value=bos_id)
        finished = paddle.fill_constant(
            shape=[batch_size, 1], dtype="bool", value=0)

        ## init states (caches) for transformer
        caches = self.transformer.decoder.gen_cache(enc_output)

        for i in range(max_len):
            trg_pos = paddle.fill_constant(
                shape=trg_word.shape, dtype="int64", value=i)
            trg = self.trg_word_embedder(trg_word)
            trg_emb = paddle.scale(x=trg, scale=self.emb_dim**0.5)
            trg_pos = self.decoder_pos_encoder(trg_pos)
            trg_pos.stop_gradient = True
            trg_emb = trg_emb + trg_pos
            dec_input = paddle.nn.functional.dropout(
                trg_emb, p=self.prepostprocess_dropout,
                training=False) if self.prepostprocess_dropout else trg_emb

            logits, caches = self.transformer.decoder(
                dec_input, enc_output, None, trg_src_attn_bias, caches)
            logits = paddle.reshape(
                logits,
                shape=[-1, logits.shape[-1]], )
            logits = self.linear(logits)

            step_log_probs = paddle.log(paddle.nn.functional.softmax(logits))

            log_probs = paddle.add(x=step_log_probs, y=log_probs)

            scores = log_probs
            topk_scores, topk_indices = paddle.topk(x=scores, k=1)

            finished = paddle.logical_or(
                finished, paddle.equal(topk_indices, end_token_tensor))
            trg_word = topk_indices
            log_probs = topk_scores

            predict_ids.append(topk_indices)

            if paddle.reduce_all(finished).numpy():
                break

        predict_ids = paddle.tensor.stack(predict_ids, axis=0)
        finished_seq = paddle.transpose(predict_ids, [1, 2, 0])
        finished_scores = topk_scores

        return finished_seq, finished_scores
